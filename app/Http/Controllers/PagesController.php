   ...
   public function app()
    {
        ...
        if ($checkMail->status->name == 'Activo') { #1
            ...
            if ($checkMail->status->name =='Pendiente') {
                ...
                User::where('email', Auth::user()->email)->update([ 'rol_uID' => $value->rol_uID, 'camp_uID' => $valueCampus->camp_uID, 'sta_uID' => $valueStatus->sta_uID, 'us_banner_guid' => $rowPerson->GUID, 'us_banner_id' => base64_encode($row->id_banner) ]);
                //dd($valueStatus->id);
